...
if __name__ == '__main__':
    ...
    while(1):
        ...
        for i in indexes:
            ...
            if name.strip() == "person":
                count +=1

            if count >= 2:
                if(now_time - sent_time > 6):
                    py_serial.write(b'H')
                    sent_time = now_time
                    time.sleep(0.1)
            ...
            # cv2.putText(image, "watever", (left, top - margin), cv2.FONT_HERSHEY_SIMPLEX, font_size, COLORS[class_id], args.thickness)
            cv2.putText(image, name, (left, top - margin),  cv2.FONT_HERSHEY_SIMPLEX, font_size, COLORS[class_id], args.thickness)
            cv2.putText(image, "people are " + str(count), (30,30),cv2.FONT_HERSHEY_SIMPLEX, font_size, COLORS[class_id], args.thickness)
